...
SELECT exams.id_exam, _subject.name FROM exams, _subject, _groups WHERE _subject.id_subject = exams.id_subject AND exams.id_group = _groups.id_group INTO CURSOR _exam

-- Берем оценки групп факультета
SELECT _students.id_group, marks_list.mark FROM marks_list, _students, _exam WHERE _students.id_student = marks_list.id_student AND marks_list.id_exam = _exam.id_exam INTO CURSOR _groups_subj
